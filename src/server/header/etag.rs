...
impl ETag {
  ...
  pub fn from_metadata(meta: &Metadata) -> Result<Self, ()> {
    let time_created = meta.created().unwrap_or(SystemTime::now())
      .duration_since(UNIX_EPOCH)
      .unwrap();

    let time_modified = meta.modified().unwrap_or(SystemTime::now())
      .duration_since(UNIX_EPOCH)
      .unwrap();

    let tag = format!("{:x}{:x}{:x}", time_created.subsec_millis(), time_modified.subsec_millis(), meta.len());
